...
export default function LegalModal({ type, onClose }) {
    ...
    return (
        <div className="fixed inset-0 z-[600] flex items-center justify-center px-4" onClick={onClose}>
            ...
            <div
                className="relative bg-gray-800 rounded-xl border border-white/10 w-full max-w-lg shadow-2xl flex flex-col max-h-[80vh]"
                onClick={e => e.stopPropagation()}
            >
                {/* Header */}
                <div className="flex items-center justify-between px-6 py-4 border-b border-white/10 shrink-0">
                    <div>
                        <h2 className="text-white font-semibold">{lang.title}</h2>
                        <p className="text-white/30 text-xs mt-0.5">{lang.updated}</p>
                    </div>
